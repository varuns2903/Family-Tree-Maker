...
const TreeEditor = () => {
  ...
  // ✅ UPDATED: Calculates Birthdays AND Anniversaries
  const calculateStats = () => {
    ...
    // 1. Birthdays
    const birthdays = nodes
      .filter(
        (n) =>
          n.isAlive &&
          n.birthDate &&
          new Date(n.birthDate).getMonth() === currentMonth
      )
      .map((n) => ({
        id: n.id,
        name: n.name,
    ...
    // 2. Anniversaries (Merge couples to avoid duplicates)
    const anniversaries = [];
    const processedCouples = new Set();

    nodes.forEach((node) => {
      if (node.weddings && node.weddings.length > 0) {
        node.weddings.forEach((w) => {
          if (!w.date) return;
          const wDate = new Date(w.date);

          if (wDate.getMonth() === currentMonth) {
            // Create a unique key for the couple (sorted IDs)
            const spouseId =
              typeof w.spouseId === "object" ? w.spouseId : String(w.spouseId);
            const coupleKey = [String(node.id), String(spouseId)]
              .sort()
              .join("_");

            if (!processedCouples.has(coupleKey)) {
              const spouse = nodes.find(
                (n) => String(n.id) === String(spouseId)
              );
              const years = new Date().getFullYear() - wDate.getFullYear();

              // Only push if years > 0 (don't show future weddings as anniversaries yet)
              if (years >= 0) {
                anniversaries.push({
                  id: node.id, // ID to focus when clicked
                  name: `${node.name.split(" ")[0]} & ${
                    spouse ? spouse.name.split(" ")[0] : "Spouse"
                  }`,
                  day: wDate.getDate(),
                  years: years, // It is the Nth anniversary
                  type: "anniversary",
                });
    ...
    // 3. Merge & Sort by Day
    const upcomingEvents = [...birthdays, ...anniversaries].sort(
      (a, b) => a.day - b.day
    );
  ...
  // ✅ NEW: Handle clicking an event in Stats Modal
  const handleStatsNodeClick = (nodeId) => {
    ...
    if (node) {
      ...
      setTimeout(() => {
        ...
        // Center the tree on this node if method available
        if (treeRef.current && typeof treeRef.current.center === "function") {
          treeRef.current.center(node.id);
        }
      }, 150);
    } else {
      toast.error("Member not found in current view");
    }
  };
  ...
  const loadTreeData = async () => {
    ...
    try {
      ...
      let fetchedNodes = membersRes.data.role
  ...
  const handleAddNewMember = async (newMemberData) => {
    if (
      ...
      !newMemberData.isAlive &&
      newMemberData.deathDate
    ) {
      if (
        new Date(newMemberData.birthDate) > new Date(newMemberData.deathDate)
      ) {
        toast.error(
    ...
    if (divRef.current) {
      ...
      if (nodes.length > 0) {
        const enriched = nodes.map((n) => ({
          ...n,
          ageDisplay:
            n.isAlive && n.birthDate
              ? `${calculateAge(n.birthDate)} yrs`
              : n.birthDate
              ? `Born ${new Date(n.birthDate).getFullYear()}`
              : "",
        }));
  ...
  return (
    <div
      className={`w-full h-screen flex overflow-hidden relative transition-colors duration-300
      ${
    ...
    >
      ...
      <div className="absolute top-4 left-4 right-4 z-30 flex justify-between items-center pointer-events-none">
        ...
        <div className="pointer-events-auto flex items-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <div
              className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1 shadow-sm ${
                userRole === "owner"
        ...
        {/* Title */}
        <div
          className={`absolute left-1/2 -translate-x-1/2 pointer-events-auto flex items-center gap-2 px-6 py-2 rounded-xl shadow-lg border backdrop-blur-md ${
            isDarkMode
              ? "bg-slate-900/80 border-slate-700 text-gray-100"
              : "bg-white/80 border-gray-200 text-gray-800"
          }`}
        >
          <TreePine size={18} className="text-green-500" />
          <span className="font-bold text-lg max-w-[150px] sm:max-w-[200px] truncate">
            {treeName}
          </span>
        </div>
        ...
        <div className="pointer-events-auto hidden md:flex gap-3">
          <div className="relative group">
            <div
              className={`flex items-center gap-2 px-4 py-2 rounded-lg cursor-pointer transition shadow-md ${
                isDarkMode
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition shadow-md ${
              isDarkMode
                ...
                : "bg-white/90 hover:bg-gray-200 text-gray-700"
            }`}
          >
            {isDarkMode ? (
              <Moon size={18} />
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition shadow-md ${
              isDarkMode
                ? "bg-slate-800 text-gray-200 hover:bg-slate-700 hover:text-blue-400"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition shadow-md ${
              isDarkMode
                ? "bg-slate-800 text-gray-200 hover:bg-slate-700 hover:text-green-400"
          ...
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg transition shadow-md ${
              isDarkMode
                ? "bg-slate-800 text-gray-200 hover:bg-slate-700 hover:text-purple-400"
                : "bg-white/90 text-gray-700 hover:bg-gray-200 hover:text-purple-600"
            }`}
          >
            <List size={18} />{" "}
      ...
      {mobileMenuOpen && (
        <div className="absolute inset-0 z-[100] flex">
          ...
          />
          <div
            className={`relative w-64 h-full shadow-2xl p-6 flex flex-col gap-2 transform transition-transform duration-300 ease-in-out ${
              isDarkMode
                ? "bg-slate-900 text-gray-100 border-r border-slate-700"
                : "bg-white text-gray-800 border-r border-gray-200"
            }`}
          >
            <div className="flex justify-between items-center mb-6">
              <span className="font-bold text-lg flex items-center gap-2">
                <TreePine className="text-green-500" /> Menu
              </span>
      ...
      <div
        ...
        className={`flex-1 w-full h-full ${
          isDarkMode ? "bg-[#1b1b1b]" : "bg-gray-50"
        }`}
      />
      ...
      <StatsModal
        ...
        onNodeClick={handleStatsNodeClick} // ✅ Pass Handler
      />
      {showShareModal && (
        <ShareModal treeId={treeId} onClose={() => setShowShareModal(false)} />
      )}
      ...
      {nodes.length === 0 && !loading && (
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-10">
          {canEdit ? (
            ...
            <div
              className={`p-6 rounded-xl text-center pointer-events-auto shadow-lg border ${
                isDarkMode
            ...
            >
              ...
              <p className="text-sm opacity-75 mb-4">
                You do not have permission to add the first member.
              </p>
              <button
