...
import { X, User, Mail, Lock, Save, LogOut, ShieldCheck } from 'lucide-react';
...
const ProfileModal = ({ user, onClose, onLogout, onUpdateUser }) => {
  ...
  const inputClass = `w-full p-3 pl-10 rounded-xl border outline-none focus:ring-2 focus:ring-blue-500 transition-all ${
    isDarkMode 
  ...
  const handleProfileUpdate = async (e) => {
    ...
    setIsLoading(true);
    try {
      // Assuming backend route PUT /auth/profile exists
  ...
  const handlePasswordUpdate = async (e) => {
    ...
    setIsLoading(true);
    try {
      // Assuming backend route PUT /auth/password exists
  ...
  return (
    ...
    >
      <div 
        className={`w-full max-w-md rounded-3xl shadow-2xl overflow-hidden transform transition-all scale-100 flex flex-col max-h-[90vh]
      ...
      >
        ...
        {/* Tabs */}
        <div className={`flex p-2 gap-2 border-b ${isDarkMode ? 'border-slate-800' : 'border-gray-100'}`}>
          <button 
            onClick={() => setActiveTab('profile')}
            className={`flex-1 py-2 text-sm font-bold rounded-xl transition ${activeTab === 'profile' 
          ...
          <button 
            ...
            className={`flex-1 py-2 text-sm font-bold rounded-xl transition ${activeTab === 'security' 
              ? (isDarkMode ? 'bg-slate-800 text-white' : 'bg-blue-50 text-blue-600') 
              : 'opacity-60 hover:opacity-100'}`}
          >
            Security
          ...
          {activeTab === 'profile' ? (
            <form onSubmit={handleProfileUpdate} className="space-y-5">
              ...
              <div className="flex flex-col items-center mb-6">
                <div className={`w-20 h-20 rounded-full flex items-center justify-center text-3xl font-bold mb-3 shadow-lg
                  ${isDarkMode ? 'bg-blue-900 text-blue-300' : 'bg-blue-100 text-blue-600'}`}>
                  {user.name.charAt(0).toUpperCase()}
                </div>
                <p className="text-xs opacity-60 uppercase tracking-widest font-bold">Personal Account</p>
              </div>
              ...
              <button 
                disabled={isLoading}
                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 mt-4 transition shadow-lg shadow-blue-500/20"
              >
            ...
            <form onSubmit={handlePasswordUpdate} className="space-y-5">
              <div className={`p-4 rounded-xl mb-4 text-sm flex items-start gap-3
        ...
        {/* Footer (Logout) */}
        <div className={`p-4 border-t ${isDarkMode ? 'border-slate-800 bg-slate-800/50' : 'border-gray-100 bg-gray-50'}`}>
          <button 
            onClick={onLogout}
